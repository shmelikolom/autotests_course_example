...
def treatment_sum(our_tuple):  # Функция сложения дфух элементов
    if len(our_tuple) > 2:  # Если переадли больше чем два элемента
        raise Exception('Много данных')  # Бросаем исключение Exception('Много данных')
    try:
        return our_tuple[0] + our_tuple[1]  # Возвращаем результат сложения двух элеметов
    except TypeError:
        # Если сложение элементов не возможно, возращаем 'Нельзя сложить эти данные'
        return 'Нельзя сложить эти данные'
    except IndexError:
        # Если элементов меньше двух, возвращает строку 'Недостаточно данных'
