...
def repeats(our_str):
    ...
    new_str = ''  # Задаем строку для возврата значений
    letters_dict = {j: 1 for j in our_str}  # Словарь из букв строки c с дефолтным значением 1
    for j in range(len(our_str)):  # Цикл прочтения строки посимвольно
        new_str += f'{our_str[j]}_{letters_dict[our_str[j]]}'  # Записываем в строку букву + количество вхождений
        letters_dict[our_str[j]] += 1  # Для встретевшейстя буквы увеличиваем значение на 1
    return new_str
